...
public class TagController extends BaseController {
    ...
    @ApiOperation(value = "添加标签", notes = "需要accessToken")
    @PostMapping("/add")
    ...
    @ApiOperation(value = "分页查询标签", notes = "需要accessToken")
    @GetMapping("/page")
    ...
    @ApiOperation(value = "获取标签列表", notes = "需要accessToken")
    @GetMapping("/list")
    public ApiResponse<List<Tag>> list() {
        return createResponse(tagService.selectTagList());
    }

    @ApiOperation(value = "修改标签名",notes = "需要accessToken")
    ...
    @ApiOperation(value = "删除标签",notes = "需要accessToken,硬删除，文章冗余字段，不会对文章有影响")
